module "identity-ci" {
  source = "git::https://github.com/pagopa/terraform-azurerm-v3.git//identity_github_runner?ref=gh-identity-module"

  resource_group_name = local.container_app_environment.resource_group
  location            = "westeurope"
...
resource "null_resource" "github_runner_app_permissions_to_namespace" {
  triggers = {
    ...
    service_principal_id = module.identity-ci.user_managed_identity.object_id
...
resource "azurerm_role_assignment" "environment_terraform_storage_account" {
  ...
  principal_id         = module.identity-ci.user_managed_identity.object_id
}
...
resource "azurerm_key_vault_access_policy" "ad_kv_group_policy" {
  ...
  object_id = module.identity-ci.user_managed_identity.object_id
