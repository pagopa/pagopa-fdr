module "identity-ci" {
  source = "git::https://github.com/pagopa/terraform-azurerm-v3.git//identity-github-runner?ref=gh-identity-module"
...
resource "null_resource" "github_runner_app_permissions_to_namespace" {
  triggers = {
    ...
    service_principal_id = module.identity-ci.client_id
...
resource "azurerm_role_assignment" "environment_terraform_storage_account" {
  ...
  principal_id         = module.identity-ci.object_id
}
...
resource "azurerm_key_vault_access_policy" "ad_kv_group_policy" {
  ...
  object_id = module.identity-ci.object_id
