...
Scenario: Add payments
  ...
  When PSP adds 5 payments whose sum is 300 to the FdR named flow_name like payload
...
Scenario: Publish FdR with bad request
...
@runnable
Scenario: Unauthorized publish FdR
  Given the Publish FdR with bad request scenario executed successfully
  When PSP with invalid subscription_key request publish to fdr-microservice with payload
  Then PSP receives the HTTP status code 401 to publish request