...
#  Check Publish FdR fails
  ...
  Scenario: Check revision after publish
  ...
  Scenario: Create the same FdR
    Given an FdR flow like same_payload
      """
        {
          "fdr": "$flow_name$",
          "fdrDate": "$flow_date$",
      ...
      """
    When PSP sends create request to fdr-microservice with same_payload
    Then PSP receives the HTTP status code 201 to create request


  Scenario: Create revision 2
    Given the Check revision after publish scenario executed successfully
