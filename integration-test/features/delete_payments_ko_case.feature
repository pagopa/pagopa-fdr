...
#  Check publish FdR fails
  ...
  Scenario:FdR deletion of a payment bad request
    Given the Add payments scenario executed successfully
    And PSP sets an invalid psp
    When PSP sends del_payments request to fdr-microservice with None
    Then PSP receives the HTTP status code 400 to del_payments request

  @runnable
  Scenario:Unauthorized FdR deletion of a payment
    Given the FdR deletion of a payment bad request scenario executed successfully
    When PSP with invalid subscription_key request del_payments to fdr-microservice with payload
    Then PSP receives the HTTP status code 401 to del_payments request
