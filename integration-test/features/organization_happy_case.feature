...
#  Get all published FdR
  ...
  Scenario: Get all published FdR with same PSP
    ...
    When Organization sends get_published_org request to fdr-microservice with None
    Then Organization receives the HTTP status code 200 to get_published_org request
    And Organization receives all FdR with the same pspId in the response of get_published_org request
  ...
  Scenario: Get all published FdR since yesterday
    ...
    When Organization sends get_published_org request to fdr-microservice with None
    Then Organization receives the HTTP status code 200 to get_published_org request
    And Organization receives all FdR with pspId eq psp in the response of get_published_org request
    And Organization receives all FdR with published gt yesterday in the response of get_published_org request
